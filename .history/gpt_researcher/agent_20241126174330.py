...
async def _conduct_research_pure(
    query: str,
    agent: str,
    role: str,
    cfg: Config,
    ...
    """Pure function to conduct research"""
    if not (agent and role):
    ...
    context = await research_conductor.conduct_research()
...
class GPTResearcher:
    ...
    async def conduct_research(self):
        """Stateful wrapper around pure research function"""
        agent, role, context = await _conduct_research_pure(
            query=self.query,
            agent=self.agent,
            role=self.role,
            cfg=self.cfg,
            research_conductor=self.research_conductor,
